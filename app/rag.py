...
# import the function from the python-doten library
#reads a file called .env
load_dotenv()
client = Groq(api_key=os.getenv("GROQ_API_KEY")) #looks for the environment variables named GROQ_API_KEY

def build_prompt(question:str, chunks:list[str], metadatas:list[dict]):
    blocks = [] #holds each chunk,
    #(zip(chunks, metadatas) -- pairs each chunk with its metadata dict
...
def generate_answer(question: str, search_results:dict):
    #unpack the chroma db results dict
